...
export const metadata = {
  title: "How Forge Physical was built",
  description: "An evidence-first hardware agent built for the All Things Agentic Hackathon.",
};

export default function BuildStory() {
  return (
    <main className="story-shell">
      <nav className="story-nav"><Link href="/">← FORGE PHYSICAL</Link><span>BUILD STORY</span></nav>
      <article className="story-article">
        <p className="eyebrow">ALL THINGS AGENTIC HACKATHON</p>
        ...
        <p className="story-required">This article was created for the purpose of entering the All Things Agentic Hackathon.</p>
        ...
        <p>Forge accepts a low-voltage product request through a public FastAPI and Streamable HTTP MCP surface. The API creates an immutable Firestore build and immediately starts a Cloud Run Job. Google ADK and Gemini 3.5 Flash produce a constrained product specification; deterministic validators gate the supported catalog, voltage, pins, buses and required connections.</p>
        <p>The worker generates Arduino firmware and asks PlatformIO for an actual ESP32 binary. It materializes a Wokwi project and deterministic temperature-alarm scenario, then stores artifacts in Cloud Storage. The Build Room polls the public API so judges can follow the same Firestore-backed event trail.</p>
        ...
        <h2>What Forge does not claim</h2>
        <p>Virtual validation does not replace physical testing. Assembly, EMI/EMC and thermals remain explicitly <code>not_verified</code>. That distinction is part of the product: every green state must correspond to evidence a judge can inspect.</p>
