...
def generate_enclosure(hardware: HardwareIR, enclosure_dir: Path) -> ToolResult:
    enclosure_dir.mkdir(parents=True, exist_ok=True)
    width, depth, height, wall = 84.0, 64.0, 30.0, 2.0
    ...
    base_triangles += _box(usb_left + 12, depth - wall, wall, width - wall - (usb_left + 12), wall, height - wall)
    ...
    lid_triangles += _box(screen_x + screen_w, screen_y, lid_z, knob_x - knob_r - (screen_x + screen_w), screen_h, wall)
    lid_triangles += _box(knob_x + knob_r, screen_y, lid_z, width - (knob_x + knob_r), screen_h, wall)
    lid_triangles += _box(knob_x - knob_r, screen_y, lid_z, knob_r * 2, knob_y - knob_r - screen_y, wall)
    lid_triangles += _box(knob_x - knob_r, knob_y + knob_r, lid_z, knob_r * 2, screen_y + screen_h - (knob_y + knob_r), wall)
    ...
    return ToolResult(
        ...
        evidence={
            "files": [str(base_path), str(lid_path)], "dimensions_mm": [width, depth, height], "wall_thickness_mm": wall,
            "cutouts": {"screen_mm": [screen_w, screen_h], "knob_diameter_mm": knob_r * 2, "usb_mm": [12, 6]},
